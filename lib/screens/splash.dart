...
import 'package:active_ecommerce_seller_app/screens/main2.dart';
...
//   child: SvgPicture.asset("assets/splash_screen_logo.svg",fit: BoxFit.cover,  width: MediaQuery.of(context).size.width,
//   height: MediaQuery.of(context).size.height,),
// );
//  return CustomSplashScreen(seconds: 3,);
  }
}
...
class _CustomSplashScreenState extends State<CustomSplashScreen> {
  ...
  //   // });
  // }
 //!newly added init state function
  void initState() {
  super.initState();
  
  // Add a delay to show splash screen first
  Future.delayed(Duration(seconds: 3), () {
    access_token.load().then((value) {
      AuthHelper().fetch_and_set().then((authResult) {
        if (authResult.result != null && authResult.result!) {
          AIZRoute.pushAndRemoveAll(context, Main(),
              middleware: MailVerificationRouteMiddleware());
        } else {
          // Navigate to Login after splash screen delay
          AIZRoute.pushAndRemoveAll(context, Login());
        }
      }).catchError((_) {
        // Fallback to Login if fetch_and_set fails
        AIZRoute.pushAndRemoveAll(context, Login());
      });
    }).catchError((_) {
      // Fallback to Login if token loading fails
       AIZRoute.pushAndRemoveAll(context, Login());
    });
  });
}
